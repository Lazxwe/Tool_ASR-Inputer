...
3. **極簡操作體驗 (Minimalist UX)**：
   ...
   - 全域快捷鍵 **`ctrl_r` (右側 Control 鍵)**：按住說話、放開即自動辨識並貼上（不易與日常快捷鍵衝突）。
4. **一鍵系統健康診斷 (`--doctor`)**：
...
| 參數名稱 | 預設值 | 可選值 | 說明 |
| :--- | :--- | :--- | :--- |
| **`hotkey_mode`** | `"hold"` | `"hold"` / `"toggle"` | **觸發模式**：<br>• `"hold"`（預設推薦）：**按住不放錄音，鬆開自動結束並貼上**。<br>• `"toggle"`：**按一下開始錄音，再按一下停止並貼上**。 |
| **`hotkey`** | `"ctrl_r"` | 字串（見下方範例） | **語音熱鍵名稱**。預設為右側 Control 鍵（`ctrl_r`），支援獨立指定左/右修飾鍵。 |
...
| 按鍵類型 | 設定檔範例 | 說明 |
| :--- | :--- | :--- |
| **獨立右側控制鍵（預設推薦）** | `"ctrl_r"`（預設）, `"alt_r"`, `"shift_r"`, `"cmd_r"` | **強烈推薦單鍵長按**（不易與左手常用的 Ctrl+C/V 等系統快捷鍵衝突） |
| **獨立左側控制鍵** | `"ctrl_l"`, `"alt_l"`, `"shift_l"`, `"cmd_l"` | 指定僅左側控制鍵觸發 |
| **單一功能鍵** | `"f8"`, `"f9"`, `"f2"`, `"f12"` | 單鍵觸發 |
| **控制鍵 + 字母** | `"ctrl_r+v"`, `"alt_r+a"`, `"ctrl+shift+a"` | 標準組合鍵 |
| **控制鍵 + 特殊鍵**| `"ctrl_r+space"`, `"alt_r+space"`, `"ctrl+space"` | 快速鍵呼叫 |
| **不分左右通用鍵** | `"ctrl"`, `"alt"`, `"shift"`, `"cmd"` | 按下左邊或右邊皆可觸發 |
...
2. **按住鍵盤 `右 Control (ctrl_r)` 不放**：選單列圖示轉為 **紅色**，代表正在錄音。
3. 對著麥克風說話。
4. **手指鬆開 `右 Control`**：選單列圖示轉為 **橘色**（進行 ASR 與繁中後處理）。
5. 辨識完成後，文字自動貼上至目前游標處，圖示回復為 **藍色**。

#### 模式二：按鍵切換 (Toggle 模式)
- 按一下 `ctrl_r` 開始錄音（圖示變紅），說完後再按一下 `ctrl_r` 停止並貼上。
